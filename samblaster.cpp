/* -*- mode: C++ ; indent-tabs-mode: nil ; c-file-style: "stroustrup" -*-
    ...
    This script has been modified by Karen Wong to be used with the NUI_pipeline. 
    After downloading samblaster, replace the original samblaster.cpp with this script and compile.
...
state_t * makeState ()
{
    ...
    s->outputFile = NULL;
    s->outputFileName = (char *)"/dev/null";
    s->discordantFile = NULL;
    s->discordantFileName = (char *)"";
    s->splitterFile = NULL;
    s->splitterFileName = (char *)"";
    s->unmappedClippedFile = stdout;
    s->unmappedClippedFileName = (char *)"stdout";
    s->sigs = NULL;
    s->minNonOverlap = 20;
    s->maxSplitCount = 2;
    s->minIndelSize = 50;
...
void printUsageString()
{
    const char* useString =
        ...
        "-o --output          FILE Output sam file for all input alignments [no alignments file output].\n"
        "-d --discordantFile  FILE Output discordant read pairs to this file. [no discordant file output]\n"
        "-s --splitterFile    FILE Output split reads to this file abiding by parameters below. [no splitter file output]\n"
        "-u --unmappedFile    FILE Output unmapped/clipped reads as FASTQ to this file abiding by parameters below. [stdout].\n"
...
int main (int argc, char *argv[])
{
    ...
    if (streq(state->unmappedClippedFileName, "stdout"))
    {
    ...
    if (state->outputFile != NULL) fclose(state->outputFile);
    if (state->discordantFile != NULL) fclose(state->discordantFile);
    if (state->splitterFile != NULL) fclose(state->splitterFile);
    if (!streq(state->unmappedClippedFileName, "stdout")) fclose(state->unmappedClippedFile);
